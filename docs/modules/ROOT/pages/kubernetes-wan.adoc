...
[NOTE]
====
The code samples present WAN Replication via LoadBalancer, which may result in the communication over only one of the target members. If you need higher performance, see xref:kubernetes-external-client:ROOT:index.adoc[External Smart Client] and use such a configuration in the WAN Replication part.
====
